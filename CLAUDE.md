...
src/demos/plot.js          Canvas 绘图工具：usePlot(自适应+高分屏+watch重绘)、
                           makeView(坐标映射)、drawAxes、plotFn、drawPoint、drawLabel、调色板 C
src/demos/*.vue            每个动画一个组件，套 DemoFrame，控件放 #controls，
                           实时数值放 #readout，说明文字放 #note
src/views/concepts/*.vue   每讲一个页面，套 ConceptPage（传 slug）
docs/course-plans.md       四门待建课程的详细规划（讲次、动画构想）——新开课先读它
```
...
2. `courses.js` 里把该课 `available: false` 改为 true，补 `path` 和 `concepts` 数组
3. 路由、视图目录按微积分的模式复制（可为每门课建 `src/views/<course>/` 子目录）
4. CourseView 目前硬编码了微积分的时间线和目录——开第二门课时需要把它参数化
