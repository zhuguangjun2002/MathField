...
// 板心温度：正方形板的板心恒等于四边温度的算术平均（平均值性质的可检验推论）。
// 读数区把两者并排印出来，读者拖任何一根滑杆都能对答案。
const centerT = computed(() => field.value[((N - 1) / 2) * N + (N - 1) / 2])
...
<template>
  <DemoFrame title="松弛法：每点不断取邻居平均，直到「无话可说」">
    ...
    <template #controls>
      <ControlSlider label="迭代次数" v-model="iters" :min="0" :max="4000" :step="20" />
    ...
    <template #readout>
      最大残差 <b>{{ fmt(residual, 3) }}</b>°&nbsp;&nbsp;板心温度 <b>{{ fmt(centerT, 2) }}</b>°&nbsp;&nbsp;四边平均
      <b>{{ fmt((topT + rightT + bottomT) / 4, 2) }}</b>°（板心理论值）
    </template>
    <template #note>
      <b>四个旋钮。</b><b>迭代次数</b>不是物理量，是<b>算法跑了多少轮</b>，拖到底 4000 轮。
      另外三个是<b>上边 / 右边 / 下边的温度</b>，单位是度，各自把那条边整条钉死在这个值上；
      <b>左边固定为 0°，没有滑杆</b>（留一条不动的边，改其余三条时对比更清楚）。
      板是 41×41 的方格，内部初值全为 0。
      <br /><br />
      一块金属板，四条边各接恒温热源，内部初始一片冰冷。规则只有一条：
      <b>每一轮，每个内部点把自己改成四个邻居的平均值</b>。把迭代次数从 0 拖到 4000：
      ...
      "解析函数全息性"的物理孪生。
      <br /><br />
      <b>读数区可以对答案。</b>正方形板的<b>板心温度必定等于四条边温度的算术平均</b>：
      把方板旋转 90°，它变回自己而四条边轮换了一遍，于是四个旋转后的解加起来处处等于
      "四边之和"这个常数；板心在旋转下不动，四者相同，一除就得到平均。
      默认边界 100 / 50 / 0 / 0 的板心真值是 37.50°，拖任何一根温度滑杆，
      读数区那两个数最终都会对上。
      <br /><br />
      <b>但请当心"最大残差"这个数——它会骗你。</b>残差量的是"每点与邻居平均还差多少"，
      它掉得比真实误差<b>快得多</b>：迭代 100 轮时残差只剩 0.28°，看着像快算完了，
      可板心才 0.70°，离真值 37.5° <b>还差 36.8°——98% 的答案还没出来</b>；
      1000 轮时残差 0.009°，板心 34.73°，仍差 2.8°；
      要拖到 3000 轮以上，板心才真正稳定在 37.49°。
      <b>"每点都快等于邻居平均了"不等于"答案对了"</b>——最难磨平的是横跨整块板的
      <b>大尺度</b>偏差，而它在局部看来处处都很服帖。这正是数值分析里
      "残差小 ≠ 误差小"的经典教训，也是多重网格法要解决的问题：
      粗网格专治大尺度、细网格专治小尺度。
    </template>
  </DemoFrame>
</template>
