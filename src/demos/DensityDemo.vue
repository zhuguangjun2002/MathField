...
<template>
  <DemoFrame title="从直方图到密度曲线：概率住进面积里">
    ...
    <template #note>
      ...
      两者该贴得很近（4000 个样本，把 b 从 0 到 4 逐档比过来，
      最大的一次也只差 <b>0.0069</b>，出现在 b = 0.55；b = 1.5 处只差 0.0004）。
      ...
      F 是本体，密度 f 只是它的导数。
      <br /><br />
      <b>照着做一遍：把组数拖到两端，各看一个数。</b>
      ① <b>组数 = 4</b>（最左）：只剩四根粗柱，形状全丢了。可把 b 拖到 <b>1.5</b>，
      读数区仍是 F(b) = 0.7914、样本频率 0.791——<b>直方图画得粗不粗，不影响它记的账</b>，
      只影响你能看出多少细节。这四根柱子里最高的那根高 <b>0.646</b>（0 到 1 之间装了 2584 个样本，
      占 64.6%，而它的宽正好是 1，所以高度和比例数值相同）；四根柱子的面积之和照旧是 1。
      ② <b>组数 = 100</b>（最右）：每根柱子只有 0.04 宽，轮廓开始发毛、上下抖动——
      这不是密度真的在抖，是样本被切得太碎：平均每箱只剩 40 个样本，
      而<b>右半边更惨：x &gt; 3.2 的那二十箱一共只落了 101 个样本，平均一箱 5 个，最少的一箱只有 2 个</b>，
      所以毛刺也是右边最凶（最左边那箱有 178 个，反倒稳）。
