...
<template>
  <ConceptPage slug="elimination">
    ...
    >
      ...
      <p>
        <strong>第一步：不许出分数，就先放大再相减。</strong>要消掉第二行的首项 2，
        把第二行整体<strong>乘 3</strong>（这是"遍乘"），第一行乘 2，再相减（这是"直除"，
        古汉语的"除"就是"减去"）：
      </p>
    ...
    >
      ...
      <p>
        <strong>第一步：容易的那一半。</strong>观察三种变换的共同点——
    ...
    >
      ...
      <p>
        ...
        它是<strong>自由变量</strong>：随便取 <MathInline tex="z=t" />，
    ...
    >
      ...
      <p><strong>第三步：乘回去。</strong>把 L 和 U 相乘，</p>
    ...
    >
      ...
      <p><strong>第二步：所有列加起来。</strong></p>
      <MathBlock tex="\sum_{k=1}^{n-1}(n-k)^2 = (n-1)^2 + (n-2)^2 + \cdots + 2^2 + 1^2 = \sum_{j=1}^{n-1} j^2" />
      <p>
        <strong>第三步：这个和你算过。</strong>正是
        <router-link to="/calculus/ftc">微积分第四讲</router-link>里用裂项推出来的那条公式
        （那里用它按定义算 <MathInline tex="\int_0^1 x^2" />）：
      </p>
    ...
    <div class="insight">
      ...
      <p>
        ...
        <router-link to="/calculus/derivative">导数那一讲</router-link>说的"局部用线性函数冒充自己"，
