...
<template>
  <ConceptPage slug="linear-system">
    ...
    <div class="story">
      <div class="story-title">📜 1948 年，图灵问了一个让所有人不安的问题</div>
      <p>
        这份不安不是今天才有的，它和电子计算机本身一样老。<strong>1946 年前后</strong>，
        战后的英国<strong>国家物理实验室（NPL）</strong>里，<strong>阿兰·图灵</strong>正在主持设计
        一台叫 <strong>ACE</strong> 的计算机。在硬件图纸之外，另一个问题缠住了他：
        机器每做一次乘除都要四舍五入一回，而解一个 <MathInline tex="n" /> 阶方程组的消元
        大约要做 <MathInline tex="n^3/3" /> 次乘除——<MathInline tex="n=100" /> 就是三十多万次。
        <strong>三十多万次微不足道的四舍五入叠在一起，最后吐出来的那个答案，还剩几分可信？</strong>
      </p>
      <p>
        当时的空气是悲观的。1940 年代前后有几份认真的分析，给出的误差上界随着阶数<strong>急剧膨胀</strong>
        （冯·诺依曼与戈德斯坦 1947 年就专门写过一篇长文，讨论高阶矩阵求逆里的舍入误差）。
        照那种上界推下去，稍大一点的方程组就该被误差彻底淹没，以至于不少人真的怀疑
        <strong>大规模消元这条路根本走不通</strong>——而当时全世界刚刚开始造计算机，
        指望它干的头等大事恰恰就是解方程组。
      </p>
      <p>
        图灵 1948 年发表的<strong>《矩阵过程中的舍入误差》</strong>（Rounding-off Errors in
        Matrix Processes）换了个问法。他没有一头扎进"误差会累积成多大"，而是先退一步问：
        <strong>这个方程组本身，对系数的微小改动有多敏感？</strong>
        敏感度是问题自带的属性，和你用哪台机器、哪套算法统统无关。图灵给这个敏感度立了一把尺子，
        就是本讲要讲的<strong>条件数</strong>。这是历史上第一次，"这道题天生难算"从一句抱怨
        变成了一个<strong>能算出来的数</strong>。
      </p>
      <p>
        在 NPL 和图灵共事的还有一个二十多岁的年轻人，<strong>詹姆斯·威尔金森</strong>，
        他一度是图灵在 ACE 项目上的助手。图灵离开 NPL 之后，威尔金森把这条路走到了底，
        发展出后来成为整个现代数值分析地基的<strong>后向误差分析</strong>：
        别问"算出的解离真解多远"，改问"我算出的这个解，恰好是<em>哪一道</em>方程组的精确解，
        那道题离我真正想解的这道，又差多远"。这句绕口的话，贰节会拆开来讲。
      </p>
      <p>
        再往前推一百多年，<strong>1809 年</strong>，高斯为了从一堆带噪声的天文观测里定出轨道，
        把消元整理成一套"不靠灵感、可以交给助手照章执行"的固定程序——就是
        <router-link to="/linear-algebra/elimination">线性代数第 1 讲</router-link>里
        把谷神星算回来的那一段，那一讲还专门许诺过："一个方程组天生有多敏感"要留到本讲来量化。
        高斯解决的是<strong>怎么算</strong>，图灵和威尔金森补上的是<strong>算出来的东西能不能信</strong>。
        这一讲要走的，正是从前者到后者的这一百四十年。
      </p>
    </div>
    <p>
      回到眼前的机器。麻烦有两种，长得像但根子完全不同。下面各摆一个能亲手复现的例子——
      两个都用 float64 实算，数字全部照抄运行结果。
    </p>
    <div class="story">
    ...
    <div class="insight">
      ...
      <p>
        ...
        （这对双竖线 <MathInline tex="\|\cdot\|" /> 读作"长度"：向量的 <MathInline tex="\|\mathbf{v}\|" />
        就是它有多长，矩阵的 <MathInline tex="\|A\|" /> 是它能把向量拉长的最大倍数；
        紧接着的下一个框会把这两件事讲清楚，肆节再给正式定义）
    ...
    <p>
      那就来定义这个"敏感度"。壹节说过，<strong>图灵 1948 年</strong>那篇论文给出的答案叫<strong>条件数</strong>。
    ...
    <QuizBox
      quiz-id="linear-system-q1"
      question="有人告诉你：「算一下行列式就知道了，$\det A$ 很小就说明方程组病态。」这个判据靠得住吗？"
      hint="先想想：把整个系数矩阵乘上一个很小的常数，行列式会变成多少，而方程组解的敏感程度变了没有。"
      :options="[
        { t: '靠得住。$\\det A$ 越接近 0，矩阵就越接近奇异，所以 $\\det$ 小就是病态、$\\det$ 不小就没事', why: '你这么选，是把「行列式小」当成了「接近奇异」。行列式是所有奇异值的乘积（至多差一个符号），它会被<strong>整体缩放</strong>糊弄：把一个 100 阶单位阵乘以 0.1，行列式立刻变成 $10^{-100}$，小得吓人——可它每个方向都被同等缩小，条件数仍然是 1，一点病都没有。反方向的反例正文里也有：$\\mathrm{diag}(100,\\,0.01)$ 的行列式恰好是 1，条件数却是 10000。' },
        { t: '靠不住。行列式量的是各个方向缩放的<strong>乘积</strong>，长短半轴可以互相掩盖；病态问的是最长与最短之<strong>比</strong>，只能由条件数 $\\kappa=\\sigma_{\\max}/\\sigma_{\\min}$ 回答', why: '对。乘积和比值是两回事，这正是贰节那个反例要说的：把 100 和 0.01 乘起来得 1（看着完美），除一除得 10000（病得不轻）。顺带把边界情形理清楚——$\\det A$ <strong>恰好为 0</strong> 确实等价于奇异、等价于 $\\kappa=\\infty$；但「小」不是「0」，而且「小」这个词本身就没有尺度不变的含义：你把整个方程组的单位从米换成毫米，行列式就变了好几个数量级，病情却纹丝不动。条件数是个比值，天生不吃这一套。' },
        { t: '半对半错：$\\det$ 小确实能说明病态，只是 $\\det$ 大的时候也可能病态，所以两个指标都要看', why: '你这么选，是把行列式当成了「不全面但仍有效」的参考。可它连单向的结论都给不出——上面 $0.1$ 倍单位阵那个例子里，行列式已经小到 $10^{-100}$，方程组却完美良态。也就是说 $\\det$ 小既不必然病态、也不必然不病态，它在这件事上<strong>不是一把不够好的尺子，而根本不是这把尺子</strong>。真要让行列式说点什么，得先按行的长度做归一化——可你一旦开始归一化，做的其实已经是在估条件数了。' },
      ]"
      :answer="1"
    />
    ...
    <QuizBox
      quiz-id="linear-system-q2"
      question="你用的库保证「后向误差极小」：算出的解代回去，残差在 $10^{-16}$ 量级。这是不是就等于说，算出的答案接近真解？"
      hint="把贰节那条乘积式写出来看看：你最终看到的误差，是被哪两样东西的乘积卡住的。"
      :options="[
        { t: '是。解代回去几乎满足原方程，残差几乎为零，那它当然离真解很近', why: '你这么选，是把「残差小」当成了「误差小」。反例就摆在壹节那张表里：13 阶希尔伯特用的正是后向稳定的算法，残差实测在 $2\\times10^{-16}$ 量级，干净得无可挑剔，可解出来的分量是 $-3.243$，真值是 1，<strong>连正负号都不对</strong>。残差小只说明你精确地解了一道「几乎一样」的题；那道题的解和这道题的解相差多远，由 $\\kappa$ 说了算，跟残差没关系。' },
        { t: '不等于。前向误差大约是<strong>条件数乘以后向误差</strong>：后向误差小只证明算法清白，问题一旦病态，答案照样能烂到面目全非', why: '对，这正是贰节那条乘积式的用处——它把两口锅分开了。后向误差 $10^{-16}$ 是算法的成绩单，$\\kappa$ 是问题的体检报告，你最终看到的误差是两者的乘积。13 阶希尔伯特那一行就是「算法清白、问题有病、答案照烂」的标本；而肆节那条粗估（$\\kappa\\approx10^{k}$ 就吃掉约 $k$ 位）能反过来当预算表用：先算 $\\kappa$，就知道 16 位里还剩几位是真的。' },
        { t: '不等于，因为后向误差小只是这一次运行碰巧走运，换台机器、换个编译器就未必了', why: '你这么选，是把「后向稳定」当成了单次运行的偶然。它不是运气：带部分主元的高斯消元的后向稳定性是被<strong>证明</strong>出来的性质，威尔金森那套分析对（几乎）所有输入都成立，换机器换编译器照样成立——这恰恰是这套理论最值钱的地方。真正的分岔不在算法这一侧，在问题那一侧：$\\kappa$。' },
      ]"
      :answer="1"
    />

    <h2><span class="sec-no">伍</span>买到了什么：一副诊断眼光，和整个科学计算的底座</h2>
    <h3>手机导航在高楼间乱飘：条件数的日常演出</h3>
    <div class="story">
    ...
    <QuizBox
      quiz-id="linear-system-q3"
      question="手上有一个病态方程组（比如高楼峡谷里那组 GPS 观测）。换一套更聪明的算法、或者把它送上超级计算机，能不能把它算准？"
      hint="先问一句：条件数是谁的属性——是那套算法的，还是那道题自己的？"
      :options="[
        { t: '能。换成更稳的算法、或者用 128 位的更高精度重算一遍，误差被压下去，答案自然就准了', why: '你这么选，是把「病」记在了算法和机器头上。提高精度确实能把答案往前多推几位——毕竟可信位数大约是「总位数减去 $\\kappa$ 的位数」，总位数变多，剩下的就多——但那是拿资源换位数，$\\kappa$ 一位没变，阶数再往上加几阶照样崩。而高楼峡谷那档更是没救：卫星挤成一条缝时缺的是<strong>信息</strong>，不是有效数字；两颗卫星完全同向时 $\\kappa=\\infty$，多少位精度都变不出那条根本没被观测到的方向。' },
        { t: '不能真正治好。$\\kappa$ 是这个方程组<strong>自己</strong>的属性，与算法和机器无关；出路是改造问题本身——换观测几何、加正则化、或者干脆去要更多信息', why: '对。这是本讲最该带走的一句话：算法的锅和问题的病是两回事，换算法只能修前者。所以现实中的对策全在「改题」这一侧——GPS 等一颗别的方向的卫星升起来（换观测几何，把 $\\kappa$ 从几十压回个位数）、岭回归加一个 $\\lambda I$ 把最小奇异值从地板上拽起来（花一点偏差买稳定）、共轭梯度先做预条件再迭代。顺带一提，「更聪明的算法」也不是全无价值：它保证你没有额外犯壹节那种小主元的低级错误——那是把答案守在 $\\kappa$ 允许的最好水平上，而不是把 $\\kappa$ 变小。' },
        { t: '不能，而且病态方程组本来就是无解的，只能放弃这组数据', why: '你这么选，是把「敏感」当成了「无解」。病态方程组绝大多数是非奇异的，有唯一解，只是这个解对输入的抖动敏感——高楼里那组观测照样解得出坐标，只是可信半径从几米变成几十米。放弃也不是工程上的答案：GPS 接收机每天都在解病态方程组，它的做法是<strong>连同 DOP 一起报出来</strong>，让你知道这次的解该信几分；只有 $\\kappa=\\infty$（两星同向）那种真正退化的情形，才是没有唯一解、必须补信息。' },
      ]"
      :answer="1"
    />
    <h3>还买到了什么</h3>
