...
<template>
  <ConceptPage slug="quadrature">
    ...
    <p>
      翻开任何一本概率论课本的最后几页，都有一张<router-link to="/probability/distribution">标准正态分布</router-link>表。表里有一行你一定见过：
    ...
    <p>
      进去之前先约好一个记号，它从这里起会反复出现：<MathInline tex="O(h^5)" /> 读作
      <strong>"量级不超过 <MathInline tex="h^5" /> 的常数倍"</strong>，用来把一长串没写出来的高次尾巴
      整个打包。它的用法就一句话——<strong>步长 <MathInline tex="h" /> 减半，这个尾巴至少缩到三十二分之一</strong>
      （<MathInline tex="2^5=32" />），同理 <MathInline tex="O(h^4)" /> 减半缩到十六分之一。
      注意它<strong>只报量级、不报系数</strong>：前面那个常数是多少，往往说不清也不必说清，
      因为 <MathInline tex="h" /> 一小，决定胜负的就是幂次而不是常数——
      本讲全部"谁比谁快"的比较，比的都是这个幂次。
    </p>
    ...
    <QuizBox
      quiz-id="quadrature-q1"
      question="梯形法把每块的顶从平线换成了斜直线，明明把曲线描得更贴，为什么反而常常输给最笨的中点矩形？"
      hint="回到那三行带符号的实测：两个误差不只是大小不同，符号也不同。在一段向上凸的曲线上，梯形的弦落在曲线哪一侧？中点矩形的平顶又落在哪一侧？"
      :options="[
        { t: '因为梯形法只有一阶精度，而中点矩形是二阶——阶不一样，越细下去差距越大', why: '两者其实<b>同阶</b>，都是 $O(h^2)$。正文那三行实测里，$N$ 每翻一倍，两个误差都降到约 $1/4$，谁也没有甩开谁。差距不在阶上，只在系数上：梯形是 $+1/12$，中点是 $-1/24$，永远差一个固定的 $-2$ 倍。把「更准」一律理解成「阶更高」，是这一讲最容易犯的错。' },
        { t: '这只是这个被积函数碰巧如此，换一个函数梯形就会反超——两者同阶，谁大谁小要看具体函数', why: '不是碰巧。折叠框里把两者都在小区间的<b>中点</b>展开，得到 $M-I=-\\frac{f^{\\prime\\prime}(c)}{24}h^3$ 与 $T-I=+\\frac{f^{\\prime\\prime}(c)}{12}h^3$，比值恒为 $-2$——$f^{\\prime\\prime}(c)$ 在上下同时出现、约掉了，<b>与被积函数是谁无关</b>。正文三行实测的比值 $-2.067,\\,-2.016,\\,-2.004$ 正是它，$N$ 越大越干净。' },
        { t: '因为中点矩形的误差自己先抵掉了大半：平顶在中点处穿过曲线，左边少算的和右边多算的互相补偿；而梯形的弦整条都在曲线同一侧，误差一边倒地累加', why: '对，这就是「贴不贴合」不等于「误差小不小」的地方。凸段上梯形的弦整条在曲线上方，每一块都<b>一致高估</b>；中点矩形的平顶却和曲线在中点交叉，左右两半先自消一轮，只剩曲率带来的残差——恰好只有梯形的一半，符号还相反。实测（正文已算，不必重算）：同样 $N=8$，中点 $1.4\\times10^{-3}$，梯形 $2.9\\times10^{-3}$。而真正值钱的不是谁赢，是这两个误差<b>反号</b>：按 $2:1$ 加权就能把主项一起消掉，那就是辛普森。' },
      ]"
      :answer="2"
    />
    ...
    <QuizBox
      quiz-id="quadrature-q2"
      question="辛普森公式在每两个小区间上只铺一条<b>抛物线</b>（二次曲线），凭什么它对<b>三次</b>多项式也分毫不差？"
      hint="把三次那一项单独拿出来看：它关于小区间的中点是什么对称性？真值那一边积出来是多少，公式那一边的加权和又是多少？"
      :options="[
        { t: '因为三次项关于区间中点是反对称的：它在真值里积出来是 0，在 $f(a)+4f(c)+f(b)$ 这个加权和里也被两端一正一负抵掉，两边同时为 0，误差自然为 0', why: '对，这一档精度是<b>对称性白送的</b>，不是设计出来的。在 $[-1,1]$ 上验一眼：$f(x)=x^3$ 时公式给 $\\frac13(-1+4\\cdot0+1)=0$，真值也是 0；换成 $f(x)=x^4$，公式给 $\\frac23\\approx0.667$，真值 $\\frac25=0.4$，这才开始出错。所以代数精度恰好是 3，一分不多一分不少。' },
        { t: '因为抛物线有三个自由度，加上区间的两个端点，一共四个条件，正好定死一个三次多项式', why: '自由度数错了。三个点只能定死一条<b>二次</b>曲线，要定死三次多项式得凑满四个条件；而辛普森手上只有 $f(a),f(c),f(b)$ 三个函数值，端点已经算在这三个里面了，没有第四个。所以若精度真由「能定死几次」决定，它就该止步于二次。它偏偏对三次也精确，多出来的那一档得另找理由——那正是三次项的反对称性。' },
        { t: '因为权重 $1,4,1$ 是特意配出来的，配的时候就把三次多项式当条件一起解进去了', why: '配平那一步只用到了二阶项。折叠框里解的是 $\\alpha+\\beta=1$ 与 $-\\alpha/24+\\beta/12=0$ 两个方程：一个保证常数的积分对，一个消掉 $h^3$ 主项，三次多项式从头到尾没被当条件写进去。$\\frac13$ 与那个 4 就是这样定出来的，三次精确是解完之后<b>顺手发现</b>的赠品。真要靠「把三次也解进去」得添第三个方程，可未知数只有两个。' },
      ]"
      :answer="0"
    />
    ...
    <h3>亲手把统计表上的 0.9750 算出来</h3>
    <div class="story">
      <div class="story-title">🩺 五个函数值、一支笔，两分钟</div>
      <p>
      ...
      <p>
        <strong>这段背景不算本讲的先修。</strong>"中心极限定理""样本均值近似正态""95% 的把握"
        这几句都是概率课的词，完整来历在站内概率课的<router-link to="/probability/clt">中心极限定理讲</router-link>；
        没读过也只需接住一句话版本——<strong>把很多次测量取平均，平均值的抖动服从钟形曲线</strong>，
        而"95% 的把握"对应的正是查表上 <strong>0.9750</strong> 那一格。
        听不懂这段背景完全不影响下面的手算：接下来要做的事只有一件，
        <strong>算一条给定曲线下方的面积</strong>。
      </p>
      <p>
    ...
    <QuizBox
      quiz-id="quadrature-q3"
      question="上面这一路看下来，辛普森的 $h^4$ 一直压着梯形的 $h^2$。那么方法的<b>阶</b>越高，算出来就永远越准吗？"
      hint="回头看肆节那两条误差公式：除了 h 的幂次，后面还挂着一个因子。它对被积函数提了什么要求？动画里哪一档正好让这个要求破产？"
      :options="[
        { t: '是。辛普森的误差里白纸黑字写着 $h^4$，比梯形的 $h^2$ 高两阶，这是定理，对任何函数都成立', why: '定理是有前提的，而前提就挂在公式自己身上：$-\\frac{(b-a)h^4}{180}f^{(4)}(\\xi)$ 里的 $f^{(4)}(\\xi)$ 要求四阶导数<b>存在且有界</b>。前提一破，$h^4$ 这个承诺跟着作废。动画里 $\\sqrt{x+2}$ 那一档就是现场演示：它在左端点的导数已经是无穷大，$f^{(4)}(\\xi)$ 无从谈起，实测三种方法的收敛比一起塌到 2.8 左右，辛普森的四阶优势<b>完全消失</b>。' },
        { t: '不一定。高阶只是「被积函数足够光滑」时的承诺——一旦函数的高阶导数在区间上炸掉，高阶公式会被拖回和低阶一样的速度', why: '对。阶是一句<b>有条件</b>的渐近承诺，条件写在误差公式里那个 $f^{(4)}(\\xi)$ 上。$\\sqrt{x+2}$ 在 $x=-2$ 处导数无穷大，于是实测中点、梯形、辛普森的收敛比一起停在 2.8 左右（即 $2^{1.5}$，阶只剩 1.5），辛普森不再比梯形快分毫。所以选方法之前先看函数光不光滑，这比记住谁是四阶更要紧；真撞上端点奇性，出路是换元或把奇点单独处理，而不是硬堆阶数。' },
        { t: '不一定，但原因是高阶公式要算更多函数值，累积的舍入误差更大，光滑不光滑倒在其次', why: '把两件事记混了。求积做的是<b>加法</b>：被积函数同号时各项不会互相抵消，舍入误差只被摊平、不会被放大——正文实测辛普森算到 $2\\times10^{-13}$ 之后只是<b>撞上地板</b>不再下降，并不像数值微分那样掉头暴涨，$N$ 加到四百万也还在同一量级。真正能废掉高阶优势的是光滑性，$\\sqrt{x+2}$ 那一档就是例子。' },
      ]"
      :answer="1"
    />
