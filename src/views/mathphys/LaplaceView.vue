...
<template>
  <ConceptPage slug="laplace">
    ...
    <QuizBox
      quiz-id="laplace-q1"
      question="推导 Δu 的真身时，为什么一定要把左右两个邻居<b>一起</b>加进来，只取一侧不行？"
      hint="把两边的泰勒展开写在一起对比：哪些项符号相同、哪些项符号相反？相加时谁活下来了？"
      :options="[
        { t: '两侧展开式里的奇次项符号相反，一加正好成对抵消，只剩下二阶项', why: '对：这才是「取两侧」的全部好处。只取一侧的话一阶项 $hu^{\\prime}(x)$ 活着，你量到的主要是斜率而不是弯曲度，$u^{\\prime\\prime}$ 被淹没在里面。' },
        { t: '为了让结果关于中心对称，好看一些', why: '你把一个硬性的代数需求当成了审美选择。对称在这里不是为了好看，是抵消奇次项的手段——不抵消掉，剩下的主项根本不是二阶导。' },
        { t: '因为一个点的邻居本来就有两个，不能只算一半', why: '这是描述现象，不是理由。一维差商就只用一侧邻居（$(u(x+h)-u(x))/h$），照样是合法的量。问题是要量什么：量斜率一侧就够，量弯曲度才非要两侧。' },
        { t: '因为二维时有四个邻居，一维也要凑成偶数个才对应得上', why: '因果反了。二维取四个邻居，正是把一维「左右一对」的做法在两个方向上各用一次的结果；不是先有四个才回头要求一维取两个。' },
      ]"
      :answer="0"
    />
    ...
    <QuizBox
      quiz-id="laplace-q2"
      question="拉普拉斯方程的定解问题里，只有边界条件，<b>没有初始条件</b>。这是为什么？"
      hint="回头看方程本身：Δu = 0 里出现了哪几个变量？有 t 吗？"
      :options="[
        { t: '方程里根本没有时间——它不问「怎么演化到这儿的」，只问「边界定死之后内部长什么样」', why: '对：波动和热都在描述演化，所以要先说清「一开始是什么样」；而拉普拉斯方程描述的是已经稳下来的状态，定解数据就只剩边界一项。' },
        { t: '因为它的初始条件恰好总是 0，写不写都一样', why: '你默认了它也有时间轴，只是初值取了个特殊值。它压根没有时间轴：$\\Delta u = 0$ 里只有空间变量，写不出 $u(x,y,0)$ 这种东西。' },
        { t: '因为解是唯一的，多给条件会矛盾', why: '唯一性是「只给边界」这件事的<b>后果</b>，不是原因。而且逻辑上也不通：正因为没有时间，才轮不到初始条件；不是因为怕矛盾才不给。' },
        { t: '因为稳态就是热方程时间趋于无穷的极限，初始条件已经被遗忘了', why: '这句物理直觉本身是对的（也确实是热方程的长时间行为），但它解释的是「为什么稳态与初值无关」，而不是「为什么方程里不能有初始条件」。方程连 t 都没有，这一层根本还没轮到。' },
      ]"
      :answer="0"
    />
    ...
    <QuizBox
      quiz-id="laplace-q3"
      question="相册里「消除路人」把挖掉的 36 个像素填回来，最坏的一个只差 1.2 级灰度。它凭什么能猜得这么准？"
      hint="想想被填的是什么内容。如果挖掉的那块原本印着一行字，同一套算法还能猜对吗？"
      :options="[
        { t: '天空、墙面这类内容本来就近似调和——本来就等于邻居平均，算法算出来的和真相是同一个东西', why: '对：所以这不是「猜」得准，是「算」得准。反过来，最大值原理直接预告了它必然在哪儿翻车：内部本来就有极值的内容（文字、纹理、高光）不满足这个前提，填出来就会糊。' },
        { t: '因为狄利克雷问题的解唯一，所以填出来的一定是原图', why: '你把「解唯一」当成了「解正确」。唯一性只保证「满足调和 + 边界值」的答案只有一个，可原图未必是调和的——原图要是印着一行字，那唯一解照样填出一片平滑的糊。' },
        { t: '因为算法记住了周围的纹理，把它复制了过来', why: '你想的是另一类算法（基于样本块的纹理合成）。本讲这一套一个字节的原始信息都没用到，它只解方程：边界钉死，内部反复取四邻平均。' },
        { t: '因为挖掉的区域只有 36 个像素，太小了不容易出错', why: '大小不是关键，内容才是。同样挖 36 个像素，落在天空上几乎无损，落在一行文字上就会糊成一片——决定成败的是那块内容本身是不是近似调和。' },
      ]"
      :answer="0"
    />
