...
<template>
  <ConceptPage slug="pde-intro">
    ...
    <QuizBox
      quiz-id="pde-intro-q1"
      question="本节说偏导数「没有任何新运算」。这句话的确切意思是什么？"
      hint="回头看那两个比喻：照片是把时间冻住，心电图是把位置冻住。冻住之后，剩下的还是不是一个单变量函数？"
      :options="[
        { t: '求偏导时把其余变量当常数看待，用的还是单变量导数那一套规则', why: '对：所以你会算 $x^2$ 的导数，就会算 $\\partial(x^2t^3)/\\partial x = 2xt^3$——把 $t^3$ 整个当成一个系数即可。新的只是「切之前先声明按哪个方向切」这个动作。' },
        { t: '偏导数是普通导数在多变量情形下的近似，变量越多误差越大', why: '你把「切片」当成了近似手段。切片是精确的：定义式里那个被冻结的 t₀ 原封不动抄了两遍，一点没有近似。真正做近似的是把偏导数换成差商（本讲伍节理查森那一步），那是另一回事。' },
        { t: '$\\partial u/\\partial x$ 衡量的是 u 随 x 和 t 一起变化的总速率', why: '你把偏导当成了「总变化率」。恰恰相反：$\\partial$ 这个弯尾巴的 d 存在的唯一理由，就是提醒你另外那些变量被按住了、没有在动。' },
        { t: '只有当 u 能写成 f(x) 乘 g(t) 时，偏导数才有定义', why: '你把第五讲那个解方程的技巧当成了偏导数的前提。偏导数对任何二元函数都有定义——只要冻结一个变量后还能求普通导数就行；能不能拆成两个单变量函数的乘积，是解方程时才操心的事。' },
      ]"
      :answer="0"
    />
    ...
    <QuizBox
      quiz-id="pde-intro-q2"
      question="偏导数的定义式里，被冻结的那个 $t_0$ 在分子上原封不动地出现了两次。这个细节为什么是整个定义的要害？"
      hint="拿定义式跟单变量导数的定义比一比：分子上有几个东西在变？"
      :options="[
        { t: '它把「冻结其余变量」从一句比喻变成了定义的字面内容——极限里真的只有 x 一个在动', why: '对：$u(x_0+h,\\,t_0) - u(x_0,\\,t_0)$ 里只有第一个位置在变。「切片」不是打比方，它就写在定义里。' },
        { t: '它保证了 u 在这一点连续', why: '你把定义的写法当成了正则性的保证。定义只规定怎么算，什么也保证不了——多元函数里甚至存在两个偏导数都存在、函数却在该点不连续的例子。' },
        { t: '它是为了让分母上的 h 能约掉', why: '你把它跟差商的分母搞混了。分母上有 h 是一切导数定义的共同长相，跟 $t_0$ 抄不抄两遍毫无关系；真正约掉 h 的是分子里的高阶小量，那是求极限那一步的事。' },
        { t: '它说明求偏导前必须先固定一个具体时刻，所以偏导数是一个数而不是函数', why: '在某一点算出来确实是一个数，这半句对；但让 $(x_0,t_0)$ 跑遍定义域，就得到一个新的二元函数 $\\partial u/\\partial x(x,t)$。后面所有偏微分方程正是把它当函数在用——不然「一条方程顶无穷条」就无从谈起。' },
      ]"
      :answer="0"
    />
    ...
    <QuizBox
      quiz-id="pde-intro-q3"
      question="理查森 1922 年那次预报，把六小时后的气压变化算成了 145 百帕（真实情况是几乎纹丝不动）。这个离谱结果的根子在哪？"
      hint="上面明确说了三件事「性质完全不同，别混为一谈」。想一想：他一共往前走了几步？没走完几百步的话，哪几个坑还来不及咬到他？"
      :options="[
        { t: '初始数据里混进了不该有的失衡——方程没错，数值也没错', why: '对：真实大气里气压和风速时刻精确平衡，而观测带着误差，喂进去的初始场是轻微失衡的。方程忠实地把这份失衡演化成了剧烈振荡。今天的业务预报专门有一道叫「初始化」的工序，干的就是先把它滤掉。' },
        { t: '他的时间步长超出了 CFL 上限约 16 倍', why: '你把坑二当成了坑一。CFL 是「连着走很多步才会咬人」的约束——误差每步翻倍，几十步后才满屏垃圾。而理查森只往前走了一步，还来不及被它咬到。' },
        { t: '蝴蝶效应把观测误差指数式放大了', why: '你把第三件事提前了。混沌放大要好几天才显形（对大气约两周），而理查森算的是六小时。这也正是「预报只能报到一周左右」的原因，跟他这次的失败不是一回事。' },
        { t: '把偏导数换成差商这个近似本身太粗糙了', why: '你把「离散化」当成了错误来源。同一套离散化 1950 年在 ENIAC 上做出了第一次成功的数值预报——方法本身是对的，正文也说了他「方法完全正确、数字全部算对」。' },
      ]"
      :answer="0"
    />
